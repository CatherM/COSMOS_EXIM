...
# read_file("L:\\basic\\divg\\EXIM\\ImmunoHematology\\Elena Camerini\\Bioinformatics\\EC024_CJ_RNAseq\\Raw data from alignment\\rsem_count \\rsem_aligned_trimmed_SQ23034888-HD112_T48-HD112_T48_.transcript.bam")



```

Splitting dataset into day 0 and day 2 like before. 

```{r}
# Split df into 2: day_0 and day_2.
# At the same time, the columns are being sorted: CLL first, in the same order as by metabolites
RNA__FPKM_day0 <- RNA_FPKM[,c(9,6,2,5,14,15,16,17,18)]
RNA_FPKM_day2 <- RNA_FPKM[,c(3,8,4,1,10,11,12,13)]

# # Remove _T0 and _T48 parts of column names # Commented this out bc idm the T0 and T48 anymore. 
# for (col in 1:ncol(RNA__FPKM_day0)){
#   colnames(RNA__FPKM_day0)[col] <- sub("_T0", "", colnames(RNA__FPKM_day0)[col])
# }
# for (col in 1:ncol(RNA_FPKM_day2)){
#   colnames(RNA_FPKM_day2)[col] <- sub("_T48", "", colnames(RNA_FPKM_day2)[col])
# }
