...
cosmos_input <- load("../data/cosmos/native/cosmos_inputs.RData")
cosmos_input <- cosmos_inputs
load("../Factor_COSMOS/data/cosmos/cosmos_inputs.RData")
...
cosmos_inputs.RData can be found here: <https://github.com/saezlab/Factor_COSMOS/blob/no_MOFA/data/cosmos/cosmos_inputs.RData>
...
cell_line <-  "RPMI-8226" # Select cell line you want to analyze with cosmos HERE

max_depth <- 8
seconds_per_step <- 72600

cosmos_inputs[[cell_line]]$RNA <- cosmos_input[[cell_line]]$RNA
...
It is possible that the cosmosR:::filter_input_notes_not_in_pkn function filters out all metab_input or sig_input. This means none of the metabolites or signals are in the PKN. No insight can be gained from this cell line or patient using COSMOS. Please try a different cell line or patient.
List of for which cell lines this is the case:
1. none of them oops
2. 

List for which patient / donor this is the case:
1.  ? TBA. Spannend
...
test_back <- preprocess_COSMOS_metabolism_to_signaling(meta_network = meta_network[,c(1,3,2)],
                                                      ...
                                                      CARNIVAL_options = my_options)
```
If error: CheckPerbutations, check if it does work with higher maximum_network_depth (4,5,6...). 
...
write_csv(SIF_full, file = paste("../results/d", paste(max_depth, paste(cell_line, "_SIF_full.csv", sep = ""), sep = ""), sep = ""))
write_csv(ATT_full, file = paste("../results/d", paste(max_depth, paste(cell_line, "_ATT_full.csv",sep = ""), sep = ""), sep = ""))
end.time <- Sys.time()
