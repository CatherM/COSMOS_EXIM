...
This R Markdown document is a translation of the Rscripts 'Prepare_metabolomic.R', from the cosmosR (specifically, the FACTOR_COSMOS/scripts/metabolomic) github page.
...
library(pheatmap)
library(vsn)
# BiocManager::install("vsn") # in case you do not have it yet |  I did not.
library(rcompanion)
library(dplyr)
library(ggplot2)
...
Next, the triplicates are merged into one by calculating the mean of sample values??
...
Each cell line is plotted as a violin, each log2 of the mean value is plotted
...
### Reframe dataframe to Sample X Metabolite

... and do not normalize apparently
...
Chanigng them to the HMDB names. Consistency is Key. Though, would you not expect to reformat the named before removing triplicates?
