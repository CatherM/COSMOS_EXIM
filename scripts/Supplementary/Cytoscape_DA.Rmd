...
Create_Network <- function(ATT, SIF, MergedGroup){
  ...
  nodes <- nodes |>
    ...
    mutate(Activity = ifelse(Activity < 0.5 & Activity > -0.5, 0, Activity)) # only keep activity above 0.5 or below -0.5
  write.csv(nodes, file = paste(data_path, paste(MergedGroup, ".csv", sep = ""), sep = "DA_All_Nodes"))
  ...
  # Pull MoleculeType from StringDB network
  ...
  saveSession(filename = paste(data_path, MergedGroup, sep = "DA_Merged_Network_"))
  
  exportImage(paste(data_path, MergedGroup, sep = "DA_Degree_Circle_"), 'PDF') #.pdf
