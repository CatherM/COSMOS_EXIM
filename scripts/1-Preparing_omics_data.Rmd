...
This document describes the preparation of transcriptomic and metabolomic T cell data. It is based off the preprocessing documents in the FACTOR_Cosmos No_MOFA branch by Saezlab on github. This document is precedes the 2 - Preprocessing omics data.Rmd document. 
...
write.csv(RNA_FPKM_day0,file = "../ourdata/transcriptomic/day0_RNA_FPKM.csv")

write.csv(RNA_FPKM_day2,file = "../ourdata/transcriptomic/day2_RNA_FPKM.csv")
...
write.csv(RNA_FPKM_day0,file = "../ourdata/transcriptomic/day0_RNA_log2_FPKM.csv")

write.csv(RNA_FPKM_day2,file = "../ourdata/transcriptomic/day2_RNA_log2_FPKM.csv")
```
...
metabs_day0 <- merge(metabolite_matching, metabs_day0, by = "Analyte.name")
metabs_day2 <- merge(metabolite_matching, metabs_day2, by = "Analyte.name")

# Remove metabolites that do not have a matching hmdb ID
metabs_day0 <- metabs_day0[!is.na(metabs_day0$HMDB.id),]
metabs_day2 <- metabs_day2[!is.na(metabs_day2$HMDB.id),]

# Remove current rownames
rownames(metabs_day0) <- NULL
rownames(metabs_day2) <- NULL

# Change HMDB ids into new rownames
metabs_day0 <- tibble::column_to_rownames(metabs_day0, var = "HMDB.id")
metabs_day2 <- tibble::column_to_rownames(metabs_day2, var = "HMDB.id")

# Remove Analyte name column
metabs_day0 <- metabs_day0[,-which(names(metabs_day0) %in% c("Analyte.name"))]
metabs_day2 <- metabs_day2[,-which(names(metabs_day2) %in% c("Analyte.name"))]
...
names(metabs_day0) <- names(RNA_FPKM_day0)
names(metabs_day2) <- names(RNA_FPKM_day2)
...
Step 4. Saving data
```{r}
# Metabolite name to HMDB table ### YOU STILL HAVE TO CHECK THESE <3 
# AMC Path
# write.csv(metabs_day0,file = "L:\\basic\\divg\\EXIM\\ImmunoHematology\\Cathy Magnée\\Data\\CD3_HDvsCLL\\Metabolomic\\day0_metabs_hmdb.csv") ## Check if this folder is correct
# 
# write.csv(metabs_day2,file = "L:\\basic\\divg\\EXIM\\ImmunoHematology\\Cathy Magnée\\Data\\CD3_HDvsCLL\\Metabolomic\\day2_metabs_hmdb.csv")
