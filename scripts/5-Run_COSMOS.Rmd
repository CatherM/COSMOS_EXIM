...
This will happen first for T0, then for T48. 
...
The goal of the chunk below is finding out at what point (which filter values) the network creates an error, per sample. This is because manually finding this out has been annoying. For ideal filter values, the following are all true:
- A network can be created from (almost) all samples (HD and CLL, T0 and T48)
- The max_depth is low, (2-3)
- The input values are significant (towards -2 and 2)
- ?? There is a balanced amount of input values for metabolite and signalling values ?? 
...
inputs_metab <- data.frame(patient = character(), 
                           n_sig_input = numeric(), 
                           f_sig = numeric(), 
                           n_metab_input = numeric(), 
                           f_metab = numeric(),
                           stringsAsFactors = FALSE)

increments <- seq(1.0, 2.0, 0.1)
# CHATGPT GEnerated based on my old codE:

for (patient in names(cosmos_inputs)) {
  filter_m <- 1.0  # Reinitialize filter_m once per patient
  
  for (filter_s in increments) {
    # print(filter_m)
    skip_to_next <- FALSE
    
    sig_input <- numeric(0)
    metab_input <- numeric(0)
    ...
    tryCatch({
      ...
      message("Error handler invoked for patient: ", patient)
      ...
      inputs_metab <<- rbind(inputs_metab, new_row)
      
      # Increment filter_m by 0.1 after catching an error
    ...
    if (skip_to_next) {
      ...
      next  # Skip to the next iteration of filter_s if an error occurred
    }
    
    # Check if filter_m has reached the upper limit
    if (filter_m >= 2.0) {
      break  # Stop the inner loop if filter_m reaches 2.0 or higher
    }
  }
}
