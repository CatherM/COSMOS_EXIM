...
max_depth <- 3
seconds_per_step <- 20 * 3600 / 64 / 6 # Time in h * Seconds in an hour / Amount of steps / 4 bc step 3 & 4 are *3
...
The chunk below runs cosmos for both timepoints, according to the
variables you set in the chunks above. Do not forget to edit the filter
accordingly!
...
for (i in 1:2){
  ...
  for (patient in names(cosmos_inputs)){ # [c(-1,-2)]
    skip_to_next <<- F
    tryCatch({
      sig_input <- cosmos_inputs[[patient]]$TF_scores
      metab_input <- cosmos_inputs[[patient]]$metabolomic 
      ...
      filter <- "s20m15" ## This one is important!! s = signalling input, m = metabolite input filter do not put in the points or smtng
      ...
      #Stp 3
      my_options$timelimit <- seconds_per_step * 3
